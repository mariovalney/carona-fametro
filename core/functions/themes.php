 ...
 */
function theme_file_url( $filename, $echo = true, $add_version = false ) {
    $filename = BASE_URL . THEMES_DIR . '/' . THEME . '/' . $filename;

    if ( $add_version && defined( 'SCRIPT_VERSION' ) && SCRIPT_VERSION ) {
        $filename .= '?v=' . SCRIPT_VERSION;
    }

    if ( ! $echo ) return $filename;
    echo $filename;
}
